...
import bookingLocalValues from "@calcom/web/public/static/bookingFields/locale.json";
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const locale: string = query.locale ? query.locale.split("-")[0] : "en";

  if (locale != "en" && eventTypeRaw.bookingFields && Array.isArray(eventTypeRaw.bookingFields)) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const localeValues: any = { ...bookingLocalValues };

    if (localeValues[locale]) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      for (const x of eventTypeRaw.bookingFields as any) {
        if (x["label"]) {
          x["label"] = localeValues[locale][x["label"]] ? localeValues[locale][x["label"]] : x["label"];
        } else {
          x["label"] = localeValues[locale][x["defaultLabel"]] || x["label"] || "";
        }
      }
    }
  }
